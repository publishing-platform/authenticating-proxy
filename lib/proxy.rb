...
private
  ...
  def process_token_or_authenticate!(env)
    ...
    # get token from either request params or previously set cookie
    if (token = request.params.fetch("token", get_auth_bypass_cookie(env)))
      # decode token
      auth_bypass_id = process_token(token, env)
    end
    # if token is successfully decoded try to retrieve logged in user (if available), otherwise authenticate
  ...
  def set_auth_bypass_cookie(response, env)
    ...
    Rack::Utils.set_cookie_header!(
      ...
      "auth_bypass_token",
      {
        ...
        path: "/",
        domain: ".#{PublishingPlatformLocation.new.external_domain}",
      },
    )
